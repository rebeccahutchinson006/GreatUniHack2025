...
class ElevenLabsTTS:
    ...
    def filter_voices_by_characteristics(
        ...
        Example:
            ...
            >>> voices = tts.filter_voices_by_characteristics(
            ...     gender='female',
            ...     accent='british'
            ... )
        """
        ...
        for voice in voices:
            ...
            # Filter by accent (case-insensitive substring match)
            ...
            # Filter by use case
            if use_case and labels.get('use case') != use_case:
                continue
            
            # Filter by description (case-insensitive substring match)
            if description:
                voice_desc = voice.get('description', '')
                if description.lower() not in voice_desc.lower():
                    continue
    ...
    def get_voice_for_language(
        ...
        """
        Get the best voice for a specific language based on preferences.
        
        Args:
            language_code: Language code (e.g., 'ES', 'FR', 'DE', 'JA').
            preferences: Optional dict with voice preferences like:
                        {'gender': 'female', 'accent': 'british'}
        
        Returns:
            Voice dictionary if found, or None to use auto-selection.
        
        Example:
            >>> tts = ElevenLabsTTS()
            >>> # Get a female voice for Spanish
            >>> voice = tts.get_voice_for_language('ES', {'gender': 'female'})
        """
        # Default language-to-accent mapping
        accent_map = {
            'ES': 'spanish',
            'FR': 'french',
            'DE': 'german',
        ...
        # For non-English languages, prefer multilingual voices
        if language_code.upper() not in ['EN', 'EN-GB', 'EN-US']:
            multilingual = self.get_multilingual_voices()
            if multilingual:
                # Apply preferences if provided
                if preferences:
                    filtered = multilingual
        ...
        # For English or if no multilingual found, try accent-specific
        target_accent = accent_map.get(language_code.upper())
        if target_accent:
            filter_args = {'accent': target_accent}
            if preferences:
                filter_args.update(preferences)
            
            matching = self.filter_voices_by_characteristics(**filter_args)
            if matching:
                return matching[0]
        
        # Fall back to auto-selection (return None)
        return None
